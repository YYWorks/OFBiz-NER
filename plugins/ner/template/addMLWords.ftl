...
<#if security.hasPermission("NERMANAGE_VIEW", session) || security.hasPermission("NERMANAGE_ADMIN", session)>
      <button class="btn btn-ner-primary-outline btn-sm m-xl" type="button" onclick="javascript: document.location='/ner/control/addDictWords';">${uiLabelMap.NerAddDictWords}</button>
</#if>
    </p>
  </div>
<#if security.hasPermission("NERMANAGE_VIEW", session) || security.hasPermission("NERMANAGE_ADMIN", session)>
  <div class="ner-words-box" id="logindev">
      <form class="form col-lg-5 col-lg-offset-0 col-sm-5 col-sm-offset-0 col-xs-2 col-xs-offset-0" method="post" action="<@ofbizUrl>addMLWords</@ofbizUrl>" name="dictionaryform">
...
$(document).ready(function(){
    ...
    $("textarea#words").attr("placeholder", "${StringUtil.wrapString(uiLabelMap.MLWordsSample)}");
})
</script>