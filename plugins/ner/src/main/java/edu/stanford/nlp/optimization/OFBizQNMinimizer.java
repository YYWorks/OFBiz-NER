...
public class OFBizQNMinimizer extends QNMinimizer {
	...
	private int evaluateIters = 0; // Evaluate every x iterations (0 = no
									// evaluation)
	private int startEvaluateIters = 0; // starting evaluation after x
										// iterations
	private Evaluator[] evaluators; // separate set of evaluators to check how
									// optimization is going
	...
	public OFBizQNMinimizer() {
	    this((Function) null);
	}

	public OFBizQNMinimizer(int m) {
	    this(null, m);
	}

	public OFBizQNMinimizer(int m, boolean useRobustOptions) {
	    this(null, m, useRobustOptions);
	}

	public OFBizQNMinimizer(Function monitor) {
		super(monitor);
		this.monitor = monitor;
	}

	public OFBizQNMinimizer(Function monitor, int m) {
		this(monitor, m, false);
	}

	public OFBizQNMinimizer(Function monitor, int m, boolean useRobustOptions) {
		super(monitor, m, useRobustOptions);
		this.monitor = monitor;
		mem = m;
		if (useRobustOptions) {
	...
	public OFBizQNMinimizer(FloatFunction monitor) {
	...
	public class Record {
		...
		public eState toContinue() {
			...
			say(" |" + nf.format(gNormLast) + "| {" + nf.format(relNorm) + "} "
					...
					+ (evalsSize > 0 ? evals.get(evalsSize - 1).toString() : "-") + " ");
	...
	public class QNInfo {
		...
		public void setHistory(List<double[]> sList, List<double[]> yList) {
			...
			for (int i = 0; i < size; i++) {
				update(sList.get(i), yList.get(i), ArrayMath.innerProduct(yList.get(i), yList.get(i)),
						ArrayMath.innerProduct(sList.get(i), yList.get(i)), 0, 1.0);
			}
		}
		 ...
		 * by the quasi newton optimization routine.
		 *
		 ...
		 *
		 */
		public int update(double[] newX, double[] x, double[] newGrad, double[] grad, double step)
	...
	public double[] minimize(DiffFunction dfunction, double functionTolerance, double[] initial,
			int maxFunctionEvaluations) {
		return ofbizMinimize(dfunction, functionTolerance, initial, maxFunctionEvaluations, null);
	}

	@SuppressWarnings("resource")
	public double[] ofbizMinimize(DiffFunction dfunction, double functionTolerance, double[] initial,
		...
		if (useOWLQN) {
			...
			grad = pseudoGradientOWL(x, grad, dfunction); // step (1) in Galen &
															// Gao except we are
															// not computing v
															// yet
		}
		...
		do {
			try {
				sayln();
				boolean doEval = (its >= 0 && its >= startEvaluateIters && evaluateIters > 0
						&& its % evaluateIters == 0);
				its += 1;
				double newValue;
				...
				qn.update(newX, x, newGrad, grad, newPoint[a]); // step (4) in
																// Galen & Gao
																// 2007
				...
				// If you wanna call a function and do whatever with the
				// information
		...
		case TERMINATE_GRADNORM:
			sayln("OFBiz QNMinimizer terminated due to numerically zero gradient: |g| < EPS  max(1,|x|) ");
			success = true;
			break;
		case TERMINATE_RELATIVENORM:
			sayln("OFBiz QNMinimizer terminated due to sufficient decrease in gradient norms: |g|/|g0| < TOL ");
			success = true;
			break;
		case TERMINATE_AVERAGEIMPROVE:
			sayln(
					"OFBiz QNMinimizer terminated due to average improvement: | newest_val - previous_val | / |newestVal| < TOL ");
			success = true;
			break;
		case TERMINATE_MAXITR:
			sayln("OFBiz QNMinimizer terminated due to reached max iteration " + maxItr);
			success = true;
			break;
		case TERMINATE_EVALIMPROVE:
			sayln("OFBiz QNMinimizer terminated due to no improvement on eval ");
			success = true;
			x = rec.getBest();
			break;
		default:
			sayln("OFBiz QNMinimizer terminated without converging");
			success = false;
			break;
		}
	...
	private void sayln(String s) {
		if (!quiet) {
			if (report == null) {
				...
				report.println(s, InterfaceReport.FORMAT_NOTE);
			}
		}
	}
	...
	private void say(String s) {
		if (!quiet) {
			if (report == null) {
				...
				report.print(s, InterfaceReport.FORMAT_NOTE);
			}
		}
	}

	// todo [cdm 2013]: Can this be sped up by returning a Pair rather than
	// copying array?
	 ...
	 *
	 * @param use
	 *            Whether to use Orthant-wise optimization
	 * @param lambda
	...
	private double[] pseudoGradientOWL(double[] x, double[] grad, Function func) {
		Set<Integer> paramRange = initializeParamRange(func, x); // initialized
																	// below
	...
	/*
	 * lineSearchBacktrackOWL is the linesearch used for L1 regularization. it
	 * only satisfies sufficient descent not the Wolfe conditions.
	 */
	private double[] lineSearchBacktrackOWL(Function func, double[] dir, double[] x, double[] newX, double[] grad,
			double lastValue) throws MaxEvaluationsExceeded {
	 ...
	 * conditions.
	 */
	private double[] lineSearchBacktrack(Function func, double[] dir, double[] x, double[] newX, double[] grad,
			double lastValue) throws MaxEvaluationsExceeded {
	...
	private double[] lineSearchMinPack(DiffFunction dfunc, double[] dir, double[] x, double[] newX, double[] grad,
			double f0, double tol) throws MaxEvaluationsExceeded {
		double xtrapf = 4.0;
		int info = 0;
		int infoc = 1;
		bracketed = false;
		...
		if (g0 >= 0) {
			// We're looking in a direction of positive gradient. This won't
			// work.
			// set dir = -grad
			for (int i = 0; i < x.length; i++) {
				dir[i] = -grad[i];
		...
		do {
			...
			// conditions.
			if ((bracketed && (newPt[a] <= stpMin || newPt[a] >= stpMax)) || fevals >= maxFevals || infoc == 0
	 ...
	 *
	 * This function is used to find a new safe guarded step to be used for line
	 * search procedures.
	 *
	 */
	private int getStep(
			/*
			 * double[] x, double[] dir, double[] newX, double f0, double g0, //
		...
		if (newPt[f] > bestPt[f]) {
			...
			stpq = bestPt[a] + ((bestPt[g] / ((bestPt[f] - newPt[f]) / (newPt[a] - bestPt[a]) + bestPt[g])) / 2)
					* (newPt[a] - bestPt[a]);
